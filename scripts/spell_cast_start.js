...
function getGameState() {
	const AuthorityGameMode = gWorld
		.readPointer()
		.add(0x118)
		.readPointer();
...
function castSpell(Caster, SpellName, CastTarget, SpellChargeLevel) {
	...
	adhSpellManagerSetSpellChargeTier(
		SpellManager,
		SpellChargeLevel + 2
	);

	logInfo(
		`${SpellManager.add(0x280).readFloat()} - cooldownMultip...`
	);
	logInfo(
		`${SpellManager.add(0x284).readFloat()} - spell charge lv...`
	);

	return adhSpellManagerCastSpell(
		SpellManager,
		SpellClass,
		CastTarget
	);
