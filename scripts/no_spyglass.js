...
Interceptor.attach(UDH_InventoryManager_AddInventoryFromTable, {
  ...
  onLeave() {
    ...
    for (let i = 0; i < len; i++) {
      // ポインタの配列なのでサイズは固定で8
      const inventory = this.result.readPointer().add(8 * i).readPointer()
...
function FNameToString(FName) {
  const _FNameToString = new NativeFunction(base.add(0x1166e70), "void", ["pointer", "pointer"]);
...
function FTextToString(FText) {
  ...
  const _FTextToString =
    new NativeFunction(base.add(0x10af640), 'pointer', ['pointer'])
